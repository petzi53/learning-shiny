...
If you’ve made an R package before, you might notice that Shiny uses the same convention for storing files containing functions. And indeed, if you’re making a complicated app, particularly if there are multiple authors, there are substantial advantages to making a full fledged package. If you want to do this, I recommend reading the “[Engineering Shiny](https://engineering-shiny.org/)” book and using the accompanying [{**golem**} package](https://thinkr-open.github.io/golem/). We’ll touch on packages again when we talk more about testing.
...
```{{r}}
sliders <- purrr::pmap(vars, mySliderInput)
```
...
`purrr::pmap()` is a variant of the `purrr::map()` function that iterate over multiple arguments simultaneously. "Parallel" here is meant in the sense that each input is processed in parallel with the others, not in the sense of multicore computing.
:::
...
::: {.callout-tip #tip-18-function-for-long-reactive}
...
To pull out a long reactive into a separate function has two advantages:
...
Since this is now an independent function, it could live in its own file (`R/load_file.R`, say), keeping the `server()` svelte (slim). This helps keep the server function focused on the big picture of reactivity, rather than the smaller details underlying each component.
