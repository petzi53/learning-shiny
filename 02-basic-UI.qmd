...
If you want to ensure that the text has certain properties you can use `shiny::validate()`, which we’ll come back to in @sec-chap08.
...
`shiny::fileInput()` requires special handling on the server side, and is discussed in detail in @sec-chap09.
...
### Action buttons {#sec-02-action-buttons}

Let the user perform an action with `shiny::actionButton()` or `shiny::actionLink()`: Actions links and buttons are most naturally paired with `shiny::observeEvent()` or `shiny::eventReactive()` in your server function. You haven’t learned about these important functions yet, but we’ll come back to them in @sec-03-control-timing.

There are several option to customize the appearance of buttons:

- Use the class argument by using one of `"btn-primary"`, `"btn-success"`, `"btn-info"`, `"btn-warning`", or `"btn-danger"`.
- Change the size with `"btn-lg`", `"btn-sm"`, `"btn-xs`". 
- Span the entire width of the element where the button is embedded using `"btn-block"`.
...
Plots are special because they are outputs that can also act as inputs. `plotOutput()` has a number of arguments like `click`, `dblclick`, and `hover`. If you pass these a string, like `click = "plot_click"`, they’ll create a reactive input (`input$plot_click`) that you can use to handle user interaction on the plot, e.g. clicking on the plot. We’ll come back to interactive plots in Shiny in @sec-chap07.


### Downloads

You can let the user download a file with `downloadButton()` or `downloadLink()`. These require new techniques in the server function, so we’ll come back to that in @sec-chap09.
