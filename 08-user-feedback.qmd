...
### Cancelling execution with `req()` {#sec-08-cancelling-execution}
...
`req()` works by signalling a special `r glossary("conditionx", "condition")`. This special condition causes all downstream reactives and outputs to stop executing. Technically, it leaves any downstream reactive consumers in an invalidated state. We’ll come back to this terminology in @sec-chap16.
...
### Removing on completion {#sec-08-removing-on-completion}
...
### Explicit confirmation {#sec-08-explicit-confirmation}
...
- We observe events generated by the UI from `modal_confirm`. These objects aren’t created statically in the ui, but are instead dynamically added in the `server()` by `showModal()`. You’ll see that idea in much more detail in @sec-chap10.
