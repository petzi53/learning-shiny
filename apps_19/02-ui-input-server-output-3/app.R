...
## path: apps_19/02-ui-input-server-output-3/app.R
...
## Another extension: TWO MODULES
## idea still from 19.3.2 Case study: selecting a numeric variable
...
selectFilterUI <- function(id) {
  selectInput(NS(id, "filter"), "Choose filter", choices = NULL)
}

selectFilterServer <- function(id, filter = "no filter") {
...
datasetServer <- function(id, filter = "no filter") {
  moduleServer(id, function(input, output, session) {
    observe({
      updateSelectInput(session, "dataset", "Choose dataset", choices = find_datasets(filter))
    })
