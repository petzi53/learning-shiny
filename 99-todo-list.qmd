...
- [x] add glossary list and sessions_info
- [ ] revise/write all chapter objectives (overviews)
- [ ] write my own summary as "lesson learned" at the end of every chapter. (The summaries in the MS-book is very general and useless. They just mention the main sections of the chapter and function most of all as a bridge passage to the next chapter.)
...
- [ ] explain how I have changed Hadley’s first person: rephrasing (more complex) or replacing many times with `we` (much easier).
...
- [ ] reoganize modules into different R files
...
- [x] remove these test-files resp. copy them to the `_archive` folder.
...
- [ ] [Write functions to simplify the code of the third exercise in @sec-19-exercises-1]{.mark}. See also  @nte-19-writing-functions-to-simplify-code
- [ ] Elaborate modularized datasets app in @lst-19-module-datasets so that user can choose from different (allowed) filter options
...
- [ ] How to work in `shinylive` with external images: via an [URL to an external website](https://forum.posit.co/t/shinylive-r-with-renderimage/202516/10), but I believe there is another solution with local images too. Maybe the problem is related with the {**shinylive**} package (not the extension). There is already a solution. See [Shinylive with renderImage() & new base64 Quarto extension](https://forum.posit.co/t/again-shinylive-with-renderimage-new-base64-quarto-extension/203838), but I have still to implement it
- [ ] Change localization. (At the moment my Shiny UI requires a comma for the user input `121,8` instead of the English localization with a decimal point `121.8`)
