...
:::::{.my-r-code}
:::{.my-r-code-header}
:::::: {#cnj-09-multiple-uploaded-files}
: Upload multiple file on the server (not working)
::::::
:::
::::{.my-r-code-container}

::: {#lst-09-multiple-uploaded-files}
...
Book example does not show multiple uploaded files, only the last one
...
###### Example does not show multiple upoaded files

`fileInput()` in @lst-09-multiple-uploaded-files does not show multiple uploaded files. One can see only the last one. 
...
```{block2, label='uploaded-multiple-files', type='rmdsuccess'}
In the meanwhile I have found a solution for . See my posting about issue [fileInput() dosen't upload multiple files](https://github.com/hadley/mastering-shiny/issues/468#issuecomment-3049717660) in "Mastering Shiny".

In @lst-09-multiple-files-uploads I have demonstrated my solution.
...
:::::{.my-r-code}
:::{.my-r-code-header}
:::::: {#cnj-09-multiple-files-upload}
: Upload multiple files on the server (working example)
::::::
:::
::::{.my-r-code-container}

::: {#lst-09-multiple-files-uploads}
...
## file: app.R
{{< include apps_09/multiple-file-uploads/app.R >}}
```
...
### Uploading data {#sec-09-uploading-data}
...
Create an app that lets you upload a `r glossary("CSV")` file, select a variable, and then perform a `t.test()` on that variable. After the user has uploaded the csv file, you’ll need to use `updateSelectInput()` to fill in the available variables. See @sec-10-updating-inputs for details.
